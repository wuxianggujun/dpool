//
// Created by WuXiangGuJun on 2024/1/6.
//

#ifndef DPOOL_THREADPOOL_HPP
#define DPOOL_THREADPOOL_HPP
...
namespace dpool {
    ...
    public:
        ...
        // submit函数返回一个std::future对象，用户可以通过这个对象获取任务的执行结果
        ...
        auto submit(Func &&func, Ts &&... params) -> std::future<typename std::result_of<Func(Ts...)>::type> {
            ...
            // 任务被封装在一个std::packaged_task对象中，这个对象可以返回执行结果。
        ...
        void worker() {
            ...
            while (true) {
                Task task;
                {
                    ...
                    auto hasTimeout = !cv_.wait_for(uniqueLock, std::chrono::seconds(WAIT_SECONDS), [this]() {
